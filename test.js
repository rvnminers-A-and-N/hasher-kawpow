...
const evrprogpow = require('./index.js');
...
function hash(headerHashBuf, nonceBuf, blockHeight) {
    ...
    evrprogpow.hashOne(headerHashBuf, nonceBuf, blockHeight, mixOutBuf, hashOutBuf);
...
function verify(mixHashBuf, expectedHash, iterations) {
    ...
    for (let i = 0; i < iterations; i++) {
        const isValid = evrprogpow.verify(headerHashBuf, nonceBuf, blockHeight, mixHashBuf, verifyHashOutBuf);
        if (!isValid)
